...
| 视频抽轨 | 在「视频处理」页展示容器内全部字幕轨，逐轨勾选后使用 MKV `mkvextract` / MP4 `ffmpeg` 抽取；PGS/VobSub 等图像字幕仅展示、不勾选 |
...
- 输出分辨率用精简下拉（档位 + 像素，如 1080p / 1920×1080），菜单自按钮顶向下展开，切换即保存。
...
7. 需要从容器抽轨时：左侧切到 **视频处理** → 在每个视频卡片中勾选需要的字幕轨 → **开始抽轨** → 再回字幕处理合并

将文件夹、字幕或视频**拖到「输入」卡片**；输出文件夹拖到「输出」卡片。设置中可改界面字体。

抽轨文件默认写到输入目录下 `dual-sub-merge-extract/`（可在设置中改名）。中文、英文轨分别使用 `chs`、`eng` 标记；同语言多轨会附加轨道编号，避免互相覆盖。
