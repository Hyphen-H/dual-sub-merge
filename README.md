...
| 输出目录 | 默认 `输入/dual-sub-merged/`；可快捷选源文件夹，或自选输出路径 |
...
4. 按需打开 **删除致谢** / **从视频抽取** / **标记语言改名**  
5. 设置分辨率与样式（菜单图标）  
6. **开始合并**（若启用标记语言则为 **改名并合并**）→ 在输出目录生成 `.chs+eng.ass`；也可先点 **仅改名**  
7. 无法可靠拆分的双语文件会跳过并在结束时列出  

也可直接把文件夹、字幕或视频**拖进窗口**（作为输入并扫描）。
